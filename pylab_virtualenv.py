...
requirements = {
    'numpy' : {
        "URL" : "http://gvis.grc.nasa.gov/sciapps/numpy-1.6.2-nosse.exe",
        "DIR" : "numpy",
        "EXTRACT" : 'numpy-1.6.2-win32-superpack-python2.7.exe',
        "INSTALLER" : 'numpy-1.6.2-nosse.exe'},

    'scipy' : {
        "URL" : "http://gvis.grc.nasa.gov/sciapps/scipy-0.10.1-nosse.exe",
        "DIR" : "scipy",
        "EXTRACT" : 'scipy-0.10.1-win32-superpack-python2.7.exe',
        "INSTALLER" : 'scipy-0.10.1-nosse.exe'},

    'matplotlib' : {
        "URL" : 'http://sourceforge.net/projects/matplotlib/files/matplotlib/matplotlib-1.1.1/matplotlib-1.1.1.win32-py2.7.exe/download',
        "DIR" : "matplotlib",
        "INSTALLER" : 'matplotlib-1.1.1.win32-py2.7.exe'},

    #TODO: Need version of distribute to install
    'distribute' : {},

    #TODO: Need version of pyreadline to install
    'pyreadline' : {},
...
#TODO: Make progress bar its own function
def writeFile(response_handle, file_handle, block_sz):
    buf = response_handle.read(block_sz)
    if buf:
        file_handle.write(buf)

    return buf

def writeStart(URL, file_size):
    if(file_size):
        print("Downloading: {0} Bytes: {1}".format(URL, file_size))
    else:
        print("Downloading: {0} Bytes: {1}".format(URL, "unknown size"))

def writeProgress(file_size_dl, file_size):
    if(file_size):
        p = float(file_size_dl) / file_size
        status = r"Bytes Downloaded: {0}  [{1:.2%}]".format(file_size_dl, p)
    else:
        status = r"Bytes Downloaded: {0}".format(file_size_dl)

    status = status + chr(8)*(len(status)+1)
    sys.stdout.write(status)

#TODO: Function for getting files via HTTP
def getFileViaHttp(URL, target, mode):
    response = urllib2.urlopen(URL)
    file_handle = open(target, mode)
    meta = response.info()
    try:
    ...
    #TODO: Function should display a progress bar
    writeStart(URL, file_size)

    file_size_dl = 0
    block_sz = 8192
    while True:
        buf = writeFile(response, file_handle, block_sz)
        if not buf:
            break

        file_size_dl += len(buf)
        writeProgress(file_size_dl, file_size)

    sys.stdout.write("\n")
    file_handle.close()
    response.close

#TODO Function for getting files via HTTPS
class VerifiedHTTPSConnection(httplib.HTTPSConnection):
    def connect(self):
        ...
        self.sock = ssl.wrap_socket(sock,
                                    ...
                                    ca_certs=os.environ['CERTS_PATH'])
...
def getFileViaHttps(URL, target, mode):
...
def after_install(options, home_dir):
    ...
    scipy = requirements['scipy']
    matplotlib = requirements['matplotlib']

    # Remember current directory
    start_dir = os.getcwd()

    # Switch to temporary directory for gathering binaries and
    # and other junk 
    os.mkdir('temp')
    os.chdir('temp')
    ...
    try:
        # Get binaries
        getFileViaHttp(numpy['URL'], numpy['INSTALLER'], "wb")
        getFileViaHttp(scipy['URL'], scipy['INSTALLER'], "wb")
        getFileViaHttp(matplotlib['URL'], matplotlib['INSTALLER'], "wb")

        # # Install python packages with easy_install
        subprocess.call([join(start_dir, home_dir, virtual_env_bin, "pip"), 'install', 'distribute'])
        subprocess.call([join(start_dir, home_dir, virtual_env_bin, "easy_install"),  numpy['INSTALLER']])
        subprocess.call([join(start_dir, home_dir, virtual_env_bin, "easy_install"), scipy['INSTALLER']])
        subprocess.call([join(start_dir, home_dir, virtual_env_bin, "pip"), 'install', 'pyreadline'])
        # #To install IPython
        subprocess.call([join(start_dir, home_dir, virtual_env_bin, "pip"), 'install', 'ipython'])
        subprocess.call([join(start_dir, home_dir, virtual_env_bin, "easy_install"), matplotlib['INSTALLER']])
